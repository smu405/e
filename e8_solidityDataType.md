...
"Out of Gas" 오류는 트랜잭션이 실행 중에 소비한 가스량이 지정된 가스 한도를 초과하여 발생하는 경우에 발생한다. 가스 한도를 초과하는 주요 원인은 다음과 같다:

- 무한 반복으로 가스를 계속 소비,
- 복잡한 연산으로 가스 소비가 증가,
- 상태 변수의 잦은 변경으로 가스 소비가 증가하는 경우

#### Http는 이벤트 리스닝 못한다.

HTTP 프로토콜은 단방향 통신을 지원하므로 이벤트 리스닝에 적합하지 않다.
